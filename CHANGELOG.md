...
- Help Menu Expansion: Updated the `-h` and `--help` flags to include instructions for the new raw capture and the Ctrl+D termination sequence.
- README Overhaul: Reorganised documentation for better navigation, moving installation guides to the top and adding a "Recent Fixes" section.

### Fixed
- Multi-line Flattening: Added logic to strip trailing backslashes and newlines when pasting commands in raw mode, ensuring vault entries remain valid one-liners.
- Quote Preservation: Resolved issues where nested quotes in curl and jq commands were being mangled by shell expansion during capture.

### Improved
- Zsh Normalisation: Refined the command cleanup process to ensure consistent spacing within the vault file regardless of the input method used.
